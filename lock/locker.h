...
public:
    Locker() {
        if (pthread_mutex_init(&mutex_, nullptr) != 0) throw std::exception();
    }
    ~Locker() {
    ...
    inline bool lock() { return pthread_mutex_lock(&mutex_) == 0; }  // 上锁
    inline bool unlock() { return pthread_mutex_unlock(&mutex_) == 0; }  // 解锁

    inline pthread_mutex_t* get() { return &mutex_; }  // 获取
...
public:
    Cond() {
        if (pthread_cond_init(&cond_, nullptr) != 0) throw std::exception();
    }
    ...
    // 唤醒一个等待的线程
    inline bool signal() { return pthread_cond_signal(&cond_) == 0; }

    // 唤醒全部等待的线程
    inline bool broadcast() { return pthread_cond_broadcast(&cond_) == 0; }
...
public:
    ...
    inline bool wait() { return sem_wait(&sem_) == 0; }

    // 增加信号量，调用一次对信号量的值+1
    inline bool post() { return sem_post(&sem_) == 0; }
