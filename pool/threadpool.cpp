...
template <typename T>
ThreadPool<T>::ThreadPool(SqlConnPool* connPool, int thread_number,
                          int max_requests)
    ...
    for (int i = 0; i < thread_number_; ++i) {
        if (pthread_create(threads_ + i, NULL, worker, this) !=
            0) {  // 函数原型中的第三个参数，为函数指针，指向处理线程函数的地址。该函数，要求为静态函数。如果处理线程函数为类成员函数时，需要将其设置为静态成员函数。
            delete[] threads_;
            // todo, log输出
            throw std::exception();
        }
