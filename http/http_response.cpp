...
bool HttpResponse::generate_response(HTTP_CODE read_ret, int &bytes_to_send,
                                     ...
                                     struct iovec &m_iv_0, struct iovec &m_iv_1,
    ...
    switch (read_ret) {
        ...
        case FILE_REQUEST:
            ...
            if (m_file_stat.st_size != 0) {
                ...
                file_address = (char *)mmap(0, (m_file_stat).st_size, PROT_READ,
                                            MAP_PRIVATE, fd, 0);
                close(fd);
...
bool HttpResponse::add_response(const char *format, ...) {
    ...
    // 从位置m_write_buf + m_write_idx 填入大小为WRITE_BUFFER_SIZE - 1 -
    // m_write_idx，格式为format， 数据为arg_list的内容
...
// 增加一个空行
bool HttpResponse::add_blank_line() { return add_response("%s", "\r\n"); }
