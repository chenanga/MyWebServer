...
LINE_STATUS HttpRequest::parse_line() {
    ...
    for (; m_checked_idx < m_read_idx; ++m_checked_idx) {
        ...
        if (temp == '\r') {
            if (m_checked_idx + 1 ==
                m_read_idx)  //表示当时数组位置已到数组末尾，但是只检测到\r，没有检测到到\n，不完整的
                return LINE_OPEN;
...
HTTP_CODE HttpRequest::parse_request_line(char *text) {
    ...
    m_url = strpbrk(text, " \t");  // strpbrk是在源字符串（s1）中找出最先含有搜索字符串（s2）中任一字符的位置并返回,
                                   // 只要包含空格或者\t就返回
    if (!m_url) {
        return BAD_REQUEST;
    }
    ...
    *m_version++ = '\0';
    ...
    // 如果直接访问ip:端口/，则跳转到初始界面,即当url为/时，显示欢迎界面, 4
    // 为欢迎界面
    if (strlen(m_url) == 1) strcat(m_url, "4");
...
HTTP_CODE HttpRequest::do_request() {
    ...
    if (m_ispost == 1 && (*(p + 1) == '2' || *(p + 1) == '3')) {
        ...
        for (name_index = name_index + 10; m_string[name_index] != '\0';
            ...
            if (users.find(name) == users.end()) {
                ...
                if (!res) {
                    ...
                    LOG_INFO(
                        ...
                        "client_address = %s",
                        name, password, inet_ntoa(m_client_address->sin_addr));
                } else {
